class alpTaxedArea
{
	int Point1[2]; // CORREÇÃO: Ponto e vírgula adicionado
	int Point2[2]; // CORREÇÃO: Ponto e vírgula adicionado
	int StockID;
	
	void alpTaxedArea( int x1, int x2, int y1, int y2, int traderID )
	{
		// CORREÇÃO: Atribuição correta de índices de array estático na Enfusion
		Point1[0] = x1;
		Point1[1] = x2;
...
{	
	...
	ref array<ref alpPlotPoleSettings> PlotPoleTaxes;
	//ref array<ref alpTimeDestrurtion> DestructionTimeInPlotPole;
	...
	*/
	...
	void InitPlotPoleTaxes()
	{
		...
		float cost, r;
		for (int i = 0; i < 20; i += 2) // OTIMIZAÇÃO: Loop iterador reescrito de forma mais legível do que i++; i++;
		{
			r = 20 + i * 5;
			cost = 2 * Math.PI * Math.Pow( r , 2 ) * ppm;
			cost = Math.Floor( cost );
			
			// CORREÇÃO: Inserção obrigatória do operador 'new'
			alpPlotPoleSettings pps = new alpPlotPoleSettings(r, i, cost, 14);
