	...
	static void ShowALLMissionsALP(MapWidget m)
	{
		...
		if ( nav ) 
		{
			...
			if ( alp_ND_base.alp_OptionsMS.ShowPlayersOnMap && nav & alpRADIO.HAS_GPS )
			{
				...
				m.AddUserMark(player.GetPosition(), player.GetIdentity().GetName(), COLOR_BLUE, "PNH_Missions\\gui\\images\\PlayerIcon.paa" );	
		...
		if ( reg_quests )
		{
			...
			if ( m_QuestMarkers /* && m_QuestMarkers.GetValueArray() && m_QuestMarkers.GetValueArray().Count() > 0 */ ) {
				...
				for(int i = 0; i<m_QuestMarkers.Count();i++){
					...
					if ( qM ) {
						m.AddUserMark( Vector(qM.X, 0,qM.Y ),qM.GetTitle() , COLOR_YELLOW, "PNH_Missions\\gui\\images\\smTrader.paa" );
					}						
				} 
				/*
			...
			{							
				...
				if ( (  (data.showInMap && data.type == ALP_MISSIONTYPEID.radiation && data.setting & alpMISSION_SETTING.RADIATION_ACTIVE ) || (data.showInMap && data.type == ALP_MISSIONTYPEID.radiationMission && data.setting & alpMISSION_SETTING.RADIATION_ACTIVE ) ) && data.position!=vector.Zero && data.missionRadius>0 )
				{
					if (data.showInMap && data.type == ALP_MISSIONTYPEID.radiation) 
						m.AddUserMark(data.position, Widget.TranslateString(data.title) + inProgress, COLOR_RED, "PNH_Missions\\gui\\images\\smRadiation.paa");	
					else 
						m.AddUserMark(data.position, Widget.TranslateString(data.title) + inProgress, COLOR_RED, "PNH_Missions\\gui\\images\\smRadiation.paa");
				...
				{
					m.AddUserMark(data.position, Widget.TranslateString(data.title) + inProgress, COLOR_GREEN, "PNH_Missions\\gui\\images\\smTrader.paa");							
				}	
				if (data.showInMap && data.type == ALP_MISSIONTYPEID.carAccident && data.position!=vector.Zero )
				{
					m.AddUserMark(data.position, Widget.TranslateString(data.title) + inProgress, ALP_COLOR_MISSION , "PNH_Missions\\gui\\images\\car.paa");							
				}											
				...
				{
					m.AddUserMark(data.position, Widget.TranslateString(data.title) + inProgress, ALP_COLOR_MISSION, "PNH_Missions\\gui\\images\\bod.paa");							
				}				
				...
				{
					m.AddUserMark(data.position, Widget.TranslateString(data.title) + inProgress, ALP_COLOR_MISSION, "PNH_Missions\\gui\\images\\mmMission.paa");								
				}
				if (data.showInMap && data.type == ALP_MISSIONTYPEID.fuelstation && data.position!=vector.Zero )
				{
					m.AddUserMark(data.position, Widget.TranslateString(data.title) + inProgress, ALP_COLOR_MISSION, "PNH_Missions\\gui\\images\\fuel.paa");								
				}		
				//new missions
				...
				{
					m.AddUserMark(data.position, Widget.TranslateString(data.title) + inProgress, ALP_COLOR_MISSION, "PNH_Missions\\gui\\images\\save.paa");								
				}	
				if (data.showInMap && data.type == ALP_MISSIONTYPEID.conquerdefend && data.position!=vector.Zero )
				{
					m.AddUserMark(data.position, Widget.TranslateString(data.title) + inProgress, ALP_COLOR_MISSION, "PNH_Missions\\gui\\images\\flag.paa");								
				}										
	...
	static void ShowResctrictedArea(MapWidget m, vector position, float radius, int color, float ratioB = 0, vector pointA = "0 0 0", vector pointB = "0 0 0")
	{
		...
		if ( !ratioB ){
			...
			{		
				...
				pos[0] = pos[0]*len;
				pos[2] = pos[2]*len;
				m.AddUserMark((position + pos) , "", color, "PNH_Missions\\gui\\images\\Bod.paa");
			...
			for (i = 0; i < pointsA;i++){
				v = pointA;
				v[0]= pointA[0] - ( i * 10 );
				m.AddUserMark(v  , "", color, "PNH_Missions\\gui\\images\\Bod.paa");	
				v[2] = pointB[2]; 
				m.AddUserMark(v  , "", color, "PNH_Missions\\gui\\images\\Bod.paa");
			}
			v = pointA;
