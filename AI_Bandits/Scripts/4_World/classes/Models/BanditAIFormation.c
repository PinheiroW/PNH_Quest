...
class BanditAIFormationVee : BanditAIFormation {
...
class BanditAIFormationWall : BanditAIFormation {
	override vector LocalFormPos(int member_no) {
		int offset = Math.Floor((member_no + 1) / 2);
		float scaled_offset = 2 * offset;
...
class BanditAIFormationFile : BanditAIFormation {
	override vector LocalFormPos(int member_no) {
		return Vector(0, 0, -2 * member_no);
	}
};

class BanditAIFormationColumn : BanditAIFormation {
	override vector LocalFormPos(int member_no) {
		int offset = Math.Floor((member_no + 1) / 2);
		float scaled_offset = 2 * offset;
