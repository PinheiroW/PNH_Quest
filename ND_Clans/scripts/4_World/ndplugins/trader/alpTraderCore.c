		...
		if ( totalbalance >= topay ) {
			if ( topay > 0 )
			{
				if ( isEbank == alpNPC_AVAILABLE_MENU.BANK_ONLY ){
					...
					// [OTIMIZAÇÃO] SaveAccount removido. O AsyncBankSave trata disto em background.
				} else  {
					if (isEbank)
					{
						topay = alpBANK.AddBalanceToAccount(guid, currency, -topay , player);
						// [OTIMIZAÇÃO] SaveAccount removido.
					}
					if ( topay )
					{
						cash -= topay;
	...
	{   
		...
		if ( totalbalance >= topay ){
			if ( topay > 0 )
			{
				...
				if (!housekey) housekey = alp_HouseKey.Cast(GetGame().CreateObject("alp_HouseKey",player.GetPosition(),false,false,true));
				...
				for(int i = 0; i < doorsCount; i++)
				{
					...
					if (!doorkey) doorkey = alp_HouseKey.Cast(GetGame().CreateObject("alp_HouseKey",player.GetPosition(),false,false,true));
