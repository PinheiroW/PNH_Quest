...
{
	// [DEBUG] Ative ou desative os logs do Trader aqui
	static const bool DEBUG_TRADER = true;

	static void TraderLog(string msg)
	{
		if (DEBUG_TRADER) Print("[PNH_TRADER_DEBUG] " + msg);
	}
	...
	static string GetCurrencyName(int id )
	{
		...
		return alpTraderCoreBase.alp_CurrencyMapped.GetElement( id );
	}
	
	alpMenuCategory GetTraderMenu()
	{
		if (IsBuyMenu() && !alp_TraderMenu) SetBuyMenu();
		if (IsSellMenu()) SetSellMenu();
	...
	int GetCount(string name, int npcID )
	{
		...
		if (stock && stock.Stock) return stock.Stock.Get( name );
		return 0;
	}
	...
	static int AddCount(int npcID, string name, int change  ) 
	{
		...
		{	
			...
			if ( centralID ) stock = GetCentralStockItems(centralID );							
			else stock = trader.Stock;			
			...
			{	
				...
				{//remove;
					if ( stock.Find( name , current ) )
					{
						...
						if (count >= 0 )
						{
							...
							TraderLog("Stock Removido -> NPC: " + npcID + " | Item: " + name + " | Qtd: " + change + " | Novo Stock: " + count);
							return Math.AbsInt( change );
						}					
						else
						{
							stock.Set( name , 0 );
							TraderLog("Stock Removido (Esgotado) -> NPC: " + npcID + " | Item: " + name);
							return current;				
						}
					}
				}
				else
				{
					...
					if ( stock.Find( name , current ) )
					{
						...
						stock.Set( name , count );
						TraderLog("Stock Adicionado -> NPC: " + npcID + " | Item: " + name + " | Adicionado: " + change + " | Novo Stock: " + count);
						return 0;
					}
					else
					{
						if ( centralID )
						{
							...
							if ( cat && count > cat.StockCap ) count = cat.StockCap;
							stock.Set( name , count );
							TraderLog("Stock Adicionado (Central) -> Item: " + name + " | Qtd: " + count);
							return 0;
						}
					}			
				}					
	...
	static map<string,int> GetCentralStockItems(int id )
	{
		...
		if ( !alp_TraderMainStock.Find(id, stock ) )
		{
			...
			if ( !stock ) stock = new map<string,int>;
	...
	int GetCurrency()
	{
		...
		if (stock) return stock.CurrencyID;
		return 0;
	}
	
	alpNPCtraderStock GetCurrentTrader()
	{
		return alp_TraderStockMapped.Get( alp_NPC_ID );
	}		
	...
	static bool AddBalanceTrader(int stockID, int change)
	{
		...
		if ( stock )
		{
			...
			{
				balance += change;
				if ( balance < 0 ) stock.Cash = 0;
				else stock.Cash = balance;
			}			
			
			TraderLog("Saldo NPC " + stockID + " Alterado -> Mudanca: " + change + " | Novo Saldo: " + stock.Cash);
			
			SaveStock(stockID);
			if ( stock.CentralStock ) SaveMainStock( stock.CentralStock );
			return true;	
		}
		return false;						
	}
	...
	{
		if (index < 0) return alp_TraderData;
	...
	void alpTraderCoreBase(map<int, ref alpMission> missions)
	{
		...
		if ( GetGame().IsServer() )
		{
			...
			ReadCategories();
			if ( missions ) GetGame().GetCallQueue(CALL_CATEGORY_GAMEPLAY).CallLater(this.InitTraders,1000,false,missions);
		}
		InitCore();
	}
	...
	array<autoptr alpNPCtraderStock> GetStocks(array<int> stockID)
	{
		...
		for (int i = 0; i < alp_TraderStockMapped.Count() ; i++)
		{
			...
			if (localStockIDs) localStockIDs.Insert( id );
			stocks.Insert( stock );
		}
		return stocks;
	}
	...
	void ReadCategories()
	{
		if ( GetGame().IsServer() ) PrintDefaultCategories();
		
		array<ref alpTraderCategories>  categories = new array<ref alpTraderCategories>;
		string path = "CfgTraders Categories"; 
		
		for(int i = 0; i < GetGame().ConfigGetChildrenCount( path ); i++)
		{
			string className;
			if (!GetGame().ConfigGetChildName(path, i, className)) continue;	
	...
	{
		if (FileExist(ALP_MISSIONS_TRADER_CATEGORIES_DEFAULT)) return;
		...
		if ( missions )
		{
			...
			foreach(alpMission mission : missions)
			{
				if (mission.GetType() == ALP_MISSIONTYPEID.trader || mission.GetType() == ALP_MISSIONTYPEID.savemerchants)
				{
					...
					if ( npcs )
					{
						foreach ( alpMissionAddNPC npc : npcs ) InitTrader(npc);
					}
					npcs = mission.GetTemplate().NPCdynamic;
					if ( npcs )
					{
						foreach ( alpMissionAddNPC npc2 : npcs ) InitTrader(npc2);
					}					
				}
			}		
		}
		...
		if ( npc.StockID > 0 )
		{
			...
			if ( FileExist(path))
			{
				...
				if ( trader.CentralStock )
				{
					...
					if (!FileExist(path2)) CreateMainStock( trader );
					else UpdateMainStock( trader );						
				}				
			}
			else
			{
				...
				if ( trader.CentralStock ) CreateMainStock( trader );
	...
	void SetDefaultItems(alpNPCtraderStock trader, array<string> tags ) 
	{
		...
		for (int i = 0; i < items.Count(); i++)
		{
			...
			if ( cat ) trader.Stock.Set( item.Name,cat.RestockCap );					
		}
	}	

	array< autoptr alpTraderItemConfig > GetTradableItems(array<string> tags = null)
	{
		if (!alp_AllItemsByTag ) InitItemList();
	...
	void InitItemList()
	{
		TraderLog("InitItemList -> Iniciando leitura CE Profile (Isto pode ser pesado na CPU)...");
		...
		{			
			if (!GetGame().ConfigGetChildName(CFG_VEHICLESPATH, i, className)) continue;	
		
			if ( (GetGame().IsKindOf( className , "Inventory_Base") || GetGame().IsKindOf( className , "CarScript")) && className !="LargeTentBackPack")
			{			
				...
				{	
					...
					if (car)
					{
						...
						catID = itm.CategoryID;
						category = GetCategory( catID );	
						if(category) // Prevent null pointer
						{
							tag = category.Tag; 
							count = category.RestockCap;
		...
		{
			if (!GetGame().ConfigGetChildName(CFG_AMMO, i, className)) continue;	
		...
		{
			if (!GetGame().ConfigGetChildName(CFG_MAGAZINESPATH, i, className)) continue;	
		...
		{
			if (!GetGame().ConfigGetChildName(CFG_WEAPONSPATH, i, className)) continue;	

			if ( GetGame().IsKindOf( className , "Weapon_Base") && (!GetGame().IsKindOf( className , "Archery_Base") || GetGame().IsKindOf( className , "Crossbow_Base")) && className != "M249"  && className != "LAW"  && className != "RPG7"  && className != "DartGun"  && className != "Shockpistol"  && className != "Shockpistol_Black") 
			{
		...
		TraderLog("InitItemList -> Leitura concluida com sucesso.");
	}	
	...
	static alpTraderCategories GetCategoryByName( string type )
	{
		...
		return GetCategory(config.CategoryID);
	}
	...
	static alpTraderItemConfig GetTraderItemConfig(string className)
	{
		...
		if ( alp_AllItemsByName && alp_AllItemsByName.Find( className , itemconfig) ) return itemconfig;
		else itemconfig = null;
		...
		if ( GetGame().IsKindOf( className , "Weapon_Base")  ) config = CFG_WEAPONSPATH + " ";
		else if ( GetGame().IsKindOf( className , "Magazine_Base")  ) config = CFG_MAGAZINESPATH + " ";
		else if ( GetGame().IsKindOf( className , "DefaultAmmo")  ) config = CFG_AMMO + " ";
		else config = CFG_VEHICLESPATH + " ";
		...
		if ( alp_ItemsOverride && alp_ItemsOverride.Count() > 0) 
		{
			...
			{
				if ( itm.Name == className ) itemconfig = itm;
			}
		}
	...
	void Restocking()
	{
		TraderLog("Restocking -> Iniciando reposicao de stock nas Lojas.");
		if (alp_TraderStockMapped)
		{
			for (int i; i < alp_TraderStockMapped.Count();i++)
			{
				...
				{
					cash += add_cash;
					if ( cash > cash_optimal) cash = cash_optimal;
					trader.Cash = cash;			
				}
				...
				for (int s = 0; s < trader.Stock.Count(); s++)
				{
					...
					if ( category)
					{
						...
						if ( restock > 0 ) {
							if ( count < stockCap){
								count += restock;								
								if ( count > stockCap) count = stockCap;	
